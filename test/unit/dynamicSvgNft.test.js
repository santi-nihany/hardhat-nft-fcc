    ...
    : describe("Dynamic SVG NFT test", function () {
          ...
          describe("Constructor", () => {
              it("initializes the NFT correctly", async () => {
                  ...
                  assert.equal(lowSvg, lowImageUri);
                  assert.equal(highSvg, highImageUri);
                  assert.equal(priceFeed, mockV3Aggregator.address);
              });
          });

          describe("Mint NFT", () => {
              it("Emits an event and creates NFT", async () => {
                  const highValue = ethers.utils.parseEther("1"); // 1 DOLLAR = 1 ETH
